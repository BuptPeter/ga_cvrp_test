...
class AGAPopulation(object):
    def __init__(self, info, total_iters,is_unique=False,is_saving=False):
        self.info = info
        self.info.max_route_len = 10
        self.chromosomes = []
        self.chromosomesHash = []
        self.is_unique = is_unique
        self.is_saving=is_saving
        chromosomes_temp = []

        if is_unique:
            while len(self.chromosomes) < 800:
                s = self.info.make_random_solution(greedy=True)
                if s.hashV not in self.chromosomesHash:
                    chromosomes_temp.append(s)
                    self.chromosomesHash.append(s.hashV)
                else:
                    print("[Init] Filter repert chromosome: ",s)
        else:
            for s in [self.info.make_random_solution(greedy=True) for _ in range(800)]:
                chromosomes_temp.append(s)
                self.chromosomesHash.append(s.hashV)
        if is_saving:
        ##【0】saving优化随机染色体
            self.chromosomesHash=[]
            for x in chromosomes_temp:
                s = self.info.steep_improve_solution(x)
                heappush(self.chromosomes, (s.cost, s))
                self.chromosomesHash.append(s.hashV)
        else:
        # 【0】无saving优化随机染色体
            for x in chromosomes_temp:
    ...
    def step(self):
        ...
        for i in range(12):
            for j in range(i + 1, 12):
                ...
                #【0】训练循环中去重
                if self.is_unique:
                    if child.hashV not in self.chromosomesHash:
                        self.chromosomesHash.append(child.hashV)
                        self.chromosomesHash.remove(self.chromosomes[-replace][1].hashV)
                        self.chromosomes[-replace] = (self.fitness(child), child)
                        replace += 1
                    else:
                        print("[Cycle] Filter repert chromosome: ", child.hashV,self.chromosomesHash)
                else:
                    self.chromosomes[-replace] = (self.fitness(child), child)
                    replace += 1
    ...
    # 需要修复时返回true，否则返回false
    #把demand最大的子段 往最小的里随机拿一个
...
class CVRPAdvancedGA(CVRPAlgorithm):#AdvancedGA:改进的遗传算法解决CVRP问题
    def __init__(self, info, num_populations, total_iters,is_unique,is_saving):
        ...
        #is_unique=True 代表染色体去重
        self.populations = [AGAPopulation(self.info, total_iters,is_unique=is_unique,is_saving=is_saving) for _ in range(num_populations)]
