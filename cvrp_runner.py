...
if __name__ == "__main__":
    ...
    # cvrp = CVRPRunner(CVRPAdvancedGA(CVRPInfo("validate/fruitybun250.vrp", debug=True), 1, 200000,False), 200000)
    # cvrp.run()
    # cvrp.write_to_file("best-solution-marking.txt")
    for i in range(10):
        print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))+" No."+str(i)+" Test log :")
        cvrp = CVRPRunner(CVRPAdvancedGA(CVRPInfo("validate/fruitybun250.vrp", debug=False), 1, 200000,False,False), 20000)
        cvrp.run()
        cvrp.write_to_file("best-solution-marking-quchong"+str(i)+".txt")